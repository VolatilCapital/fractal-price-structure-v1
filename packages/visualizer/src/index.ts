...
function printMove(move: PriceMove, indent = 0): void {
  ...
  const state = move.isActive() ? `${Colors.green}ACTIVE${Colors.reset}` : `${Colors.dim}CLOSED${Colors.reset}`
...
export class DebugVisualizer {
  ...
  constructor(engine?: FractalEngine) {
    this.engine = engine ?? new FractalEngine({
  ...
  printLayers(): void {
    ...
    for (const layer of layers) {
      console.log()
      console.log(`${Colors.bright}${Colors.yellow}  Layer ${layer.level}${Colors.reset} (${layer.moves.length} moves)`)
      printSeparator("─", 76)
...
if (process.argv[1]?.includes("visualizer")) {
  ...
  console.log()
  printSeparator("═")
  console.log(`${Colors.dim}  Visualization complete. ${candles.length} candles processed.${Colors.reset}`)
  printSeparator("═")
}
